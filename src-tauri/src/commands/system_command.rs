...
use tauri::{
    command, AppHandle, PhysicalPosition, State, WebviewUrl, WebviewWindowBuilder, Window,
};
...
pub async fn save_preference_setting(auto_start: bool, auto_update: bool) -> Result<(), String> {
    ...
    config_manager
        .update_auto_update(auto_update)
        .map_err(|e| format!("Failed to save auto_update: {}", e))?;
    Ok(())
}

#[tauri::command]
pub async fn save_git_setting(
...
#[tauri::command]
pub async fn save_path_setting(
    app: AppHandle,
    ...
    config_manager
        .update_auto_backup(auto_backup)
        .map_err(|e| format!("Failed to save auto_backup: {}", e))?;
    crate::events::system_event::emit_storage_path_changed(&app, note_storage_path_clone)
